...
pbmc <- CreateSeuratObject(counts = a, project = "GBMLGG_ATAC", min.cells = 0, min.features = 0)

pbmc@meta.data$tag=TAG

pbmc <- NormalizeData(object = pbmc, normalization.method = "LogNormalize",  scale.factor = 10000)

FNUM=round(nrow(pbmc)/100)

VAR=apply(a,1,var)

used_feature=which(VAR >=1)

VariableFeatures(object = pbmc)=rownames(pbmc)[used_feature]

length(VariableFeatures(object = pbmc))# 27380
...
DimPlot(pbmc, reduction = "pca", group.by='tag')

saveRDS(pbmc,file='newpbmc.RDS')
...
PC1_GENE=pbmc@reductions$pca@feature.loadings[,1]
GBM=which(TAG=='GBMx')
LGG=which(TAG=='LGGx')
...
PC2_PEAK_LOADING=pbmc@reductions$pca@feature.loadings[,2]
PC3_PEAK_LOADING=pbmc@reductions$pca@feature.loadings[,3]
PC4_PEAK_LOADING=pbmc@reductions$pca@feature.loadings[,4]
HC= 0.005
LC= -0.005
